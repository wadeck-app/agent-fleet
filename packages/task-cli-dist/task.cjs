...
// dist/cli/TaskIndex.js
var TaskIndex_exports = {};
__export(TaskIndex_exports, {
...
// dist/cli/commands/TaskCliCommand.js
...
function loadFlowConfig(configFile) {
	if (!fs2.existsSync(configFile)) return DEFAULT_CONFIG;
	try {
		const loaded = load(fs2.readFileSync(configFile, 'utf8'), {
			schema: JSON_SCHEMA,
		});
		return {
...
var HookDispatcher = class {
	...
	async dispatch(event, payload, onError) {
...
var TaskStore = class {
	...
	create(title) {
		...
		fs3.writeFileSync(this.taskFilePath(id), JSON.stringify(record, null, 2), 'utf8');
	...
	ensureDirectory() {
		fs3.mkdirSync(this.tasksDir, { recursive: true });
	}
	taskFilePath(id) {
		return path3.join(this.tasksDir, `${id}.json`);
	}
	indexFilePath() {
		return path3.join(this.tasksDir, 'index.json');
	}
	readIndex() {
		...
		try {
			return JSON.parse(fs3.readFileSync(indexPath, 'utf8'));
		} catch (err) {
	...
	writeIndex(index) {
		this.ensureDirectory();
		fs3.writeFileSync(this.indexFilePath(), JSON.stringify(index, null, 2), 'utf8');
	}
	addToIndex(summary) {
		const index = this.readIndex();
...
function getCurrentTaskVersion() {
	try {
		return '1.0.0';
	} catch {
...
function getUpdaterPath() {
	...
	const dir = path4.dirname(bundlePath);
	const candidates = [path4.join(dir, 'task-updater.cjs'), path4.join(dir, 'flow-updater.cjs')];
...
async function runSelfChecks() {
	...
	}
	{
		...
		try {
			const config = loadFlowConfig(path4.join(os2.tmpdir(), '.task-self-check-nonexistent-config.yaml'));
	...
	}
	{
		...
		let tmpDir;
		try {
			tmpDir = fs4.mkdtempSync(path4.join(os2.tmpdir(), 'task-self-check-'));
	...
	}
	{
		...
		try {
			const config = loadFlowConfig(path4.join(os2.tmpdir(), '.task-self-check-schema.yaml'));
...
`);
		...
		if (!quiet) {
			process.stdout
				.write(`Self-check failed (${failedCount}/${results.length} checks failed). Run: task cli update --log
`);
		}
		process.exit(1);
	}
}
async function runTaskCliVersion() {
...
async function runTaskCliUpdate(opts) {
	if (opts.log) {
		const logFile = path4.join(getConfigDir(), 'update-log.txt');
...
function runTaskCliRollback() {
	...
	const stateFile = path4.join(configDir, 'update-state.json');
...
async function runTaskCliSelfCheck() {
...
function resolveGlobalConfigDir(configDirOverride) {
	const raw = configDirOverride ?? process.env['TASK_CONFIG'] ?? path5.join(os3.homedir(), '.task');
	return expandTilde(raw);
}
function loadYamlFile(filePath) {
	if (!fs5.existsSync(filePath)) return void 0;
	const raw = fs5.readFileSync(filePath, 'utf8');
	return load(raw);
}
function isProjectInitialized(projectDir) {
	const dir = projectDir ?? process.cwd();
	return fs5.existsSync(path5.join(dir, '.task'));
}
function loadTaskConfig(options) {
	const globalConfigDir = resolveGlobalConfigDir(options?.configDir);
	const projectDir = options?.projectDir ?? process.cwd();
	const globalConfig = loadYamlFile(path5.join(globalConfigDir, 'config.yml')) ?? {};
	const projectConfig = loadYamlFile(path5.join(projectDir, '.task', 'config.yml')) ?? {};
	return {
...
// dist/cli/TaskIndex.js
var PROJECT_CONFIG_TEMPLATE = `# Task configuration for this project
...
function parseHookCommand(value) {
	...
	const debug = typeof value === 'object' ? value.debug : void 0;
	const parts = raw.trim().split(/\s+/);
	if (!parts[0]) {
		throw new Error(`Hook command cannot be empty. Got: "${raw}"`);
	}
	return { type: 'cli', command: parts[0], args: parts.slice(1), ...(debug !== void 0 && { debug }) };
}
function buildHookDispatcher(globalHooks, projectHooks) {
	const combined = {};
	const allEvents = /* @__PURE__ */ new Set([...Object.keys(globalHooks), ...Object.keys(projectHooks)]);
...
function parseArgs(args) {
	...
	let jsonMode = false;
	while (i < args.length) {
		if (args[i] === '--config' && args[i + 1] !== void 0) {
			configDir = args[i + 1];
			i += 2;
		} else if (args[i] === '--json') {
			jsonMode = true;
	...
	for (let j = 0; j < trailing.length; j++) {
		if (trailing[j] === '--project-dir' && trailing[j + 1] !== void 0) {
...
function errorOutput(jsonMode, message, hint) {
	if (jsonMode) {
...
async function runTaskCommand(args, cwd) {
...
`);
	...
	if (updateState?.status === 'rolled-back') {
		process.stderr
			.write(`[task] Update to v${updateState.targetVersion} failed (self-check failed). Rolled back to v${updateState.previousVersion}. Run: task cli update --log
`);
	}
...
`);
	...
	const projectEnv = {
		taskProjectName: path6.basename(effectiveCwd),
		...
		case 'new': {
			...
			await hookDispatcher.dispatch(
				'onTaskCreated',
				{
					...
					taskFile: path6.join(tasksDir, `${task.id}.json`),
					...projectEnv,
				},
				() =>
...
`)
		...
		case 'show': {
			...
			try {
				...
				return errorOutput(jsonMode, error.message);
			}
		}
		case 'set-status': {
			const id = rest[0];
...
${validLine}${hintLine}`,
			...
			try {
				...
				const updated = store.updateStatus(before.id, statusArg);
				await hookDispatcher.dispatch(
					'onStatusChange',
					{
						taskId: updated.id,
...
var isEntryPoint =
	...
	(process.argv[1] === (0, import_node_url2.fileURLToPath)(__importMetaUrl) ||
