...
function loadTaskConfig(options) {
	...
	const globalConfig = loadYamlFile(path.join(globalConfigDir, 'config.yml')) ?? {};
	const projectConfig = loadYamlFile(path.join(projectDir, '.task', 'config.yml')) ?? {};
	return {
...
var TaskStore = class {
	...
	create(title) {
		...
		fs2.writeFileSync(this.taskFilePath(id), JSON.stringify(record, null, 2), 'utf8');
	...
	ensureDirectory() {
		fs2.mkdirSync(this.tasksDir, { recursive: true });
	}
	taskFilePath(id) {
		return path2.join(this.tasksDir, `${id}.json`);
	}
	indexFilePath() {
		return path2.join(this.tasksDir, 'index.json');
	}
	readIndex() {
		...
		try {
			return JSON.parse(fs2.readFileSync(indexPath, 'utf8'));
		} catch (err) {
	...
	writeIndex(index) {
		this.ensureDirectory();
		fs2.writeFileSync(this.indexFilePath(), JSON.stringify(index, null, 2), 'utf8');
	}
	addToIndex(summary) {
		const index = this.readIndex();
...
function getConfigDir() {
	...
	if (xdg) return path3.join(xdg, 'flow');
	if (process.platform === 'win32') {
		const appData = process.env['APPDATA'];
		if (appData) return path3.join(appData, 'flow');
	}
	return path3.join(process.env['HOME'] ?? os2.homedir(), '.config', 'flow');
}

// dist/updater/UpdateManager.js
function scheduleBackgroundUpdate(bundlePath, pkgName, updaterName = 'flow-updater.cjs') {
	const dir = path4.dirname(bundlePath);
	const updaterPath = fs3.existsSync(path4.join(dir, updaterName))
		? path4.join(dir, updaterName)
		: fs3.existsSync(path4.join(dir, 'flow-updater.cjs'))
			? path4.join(dir, 'flow-updater.cjs')
			: null;
