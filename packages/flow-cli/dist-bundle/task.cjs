...
function buildHookDispatcher(globalHooks, projectHooks) {
  ...
  for (const event of allEvents) {
    ...
    if (globalCmd)
      list.push(parseHookCommand(globalCmd));
    const projectCmd = projectHooks[event];
    if (projectCmd)
      list.push(parseHookCommand(projectCmd));
    if (list.length > 0)
      combined[event] = list;
  }
...
async function runTaskCommand(args, cwd) {
  ...
  const effectiveCwd = effectiveProjectDir ? path3.resolve(cwd, effectiveProjectDir) : cwd;
  ...
  if (command === "init") {
    const taskDir = path3.join(effectiveCwd, ".task");
    if (fs3.existsSync(taskDir)) {
      return { exitCode: 0, output: "Project already initialized." };
    }
    fs3.mkdirSync(path3.join(taskDir, "tasks"), { recursive: true });
    fs3.writeFileSync(path3.join(taskDir, "config.yml"), PROJECT_CONFIG_TEMPLATE, "utf8");
  ...
  switch (command) {
    case "new": {
      ...
      await hookDispatcher.dispatch("onTaskCreated", {
        ...
        taskFile: path3.join(tasksDir, `${task.id}.json`),
        ...projectEnv
...
`));
    ...
    default:
      return errorOutput(jsonMode, `unknown command: ${command}`, "Valid commands: init, new, list, show, set-status");
  }
}
async function main() {
...
var isEntryPoint = process.argv[1] !== void 0 && (process.argv[1] === (0, import_node_url.fileURLToPath)(__importMetaUrl) || process.argv[1].endsWith("TaskIndex.js") || process.argv[1].endsWith("TaskIndex.ts"));
if (isEntryPoint) {
  main().catch((error) => {
