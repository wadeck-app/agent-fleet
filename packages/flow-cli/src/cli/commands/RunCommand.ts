...
type FlowResolution =
	| { found: true; flowFile: string; inferredFlowId?: string }
...
function validateSecretInputs(flowFilePath: string, inputs: Record<string, string>): string | null {
	...
	for (const [key, spec] of Object.entries(flow.inputs)) {
		...
		if (!isUriScheme) {
			return `Input '${key}' is of type '${inputType}' but received a literal value. ` +
...
function parseInputArgs(rawInputs: string[]): Record<string, string> {
	...
	for (const entry of rawInputs) {
		...
		if (idx === -1) { console.error(`Invalid input: '${entry}'. Use key=value`); process.exit(1); }
...
function loadDaemonConfig(configFile: string): typeof DEFAULT_CONFIG {
	...
	try {
		const loaded = yaml.load(fs.readFileSync(configFile, 'utf8'), { schema: yaml.JSON_SCHEMA }) as typeof DEFAULT_CONFIG;
		return {
...
export function registerRunCommand(program: Command): void {
	program
		...
		.option('-i, --input <key=value>', 'Input key=value (repeatable)', (val: string, acc: string[]) => { acc.push(val); return acc; }, [] as string[])
		...
		.action(async (flowRef: string, options: { input: string[]; flowId?: string; wait?: boolean; timeout: string; quiet?: boolean; json?: boolean; human?: boolean }) => {
			...
			if (!resolution.found) { console.error(resolution.error); process.exit(1); }
			...
			const cmd: Extract<ClientCommand, { type: 'run' }> = {
				type: 'run', flowFile, flowId, inputs, quiet: options.quiet, cwd,
			};
			...
			if (options.json && !options.human) {
				...
				process.stdout.write(JSON.stringify({ executionId, status: finalState.status, outputs: stepOutputs, durationMs }) + '\n');
			} else {
