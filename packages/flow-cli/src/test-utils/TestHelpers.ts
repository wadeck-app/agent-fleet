...
export async function startTestDaemon(): Promise<TestDaemonContext> {
    ...
    return {
        ...
        async [Symbol.asyncDispose](): Promise<void> {
            try { await handle.stop('idle'); } catch { /* ignore cleanup errors */ }
            fs.rmSync(daemonDir, { recursive: true, force: true });
        },
    };
}

export async function waitForExecution(daemonDir: string, executionId: string, timeoutMs: number): Promise<ExecutionState> {
