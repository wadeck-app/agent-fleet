import { VERSION_RE, validateVersion } from './versionValidation';
...
describe('versionValidation', () => {
	describe('VERSION_RE', () => {
		...
		it('matches build metadata: 1.0.0+build.1', () => {
			expect(VERSION_RE.test('1.0.0+build.1')).toBe(true);
		});
	...
	describe('validateVersion', () => {
		...
		it('returns pre-release version string on success', () => {
			expect(validateVersion('2026.08.20-319-abc1234')).toBe('2026.08.20-319-abc1234');
		});

		it.each(['', 'abc', '1.0', 'v1.0.0'])('throws with message "Invalid version string: ..." for: %s', v => {
			expect(() => validateVersion(v)).toThrow(`Invalid version string: "${v}"`);
		});
	});
});
