...
// ---------------------------------------------------------------------------
// T_PAYLOAD: payload validation in FlowEngine commands
// ---------------------------------------------------------------------------

describe('engine daemon payload validation', () => {
	it('run-flow throws when payload is null', async () => {
		await using daemon = await createTestDaemon({ commands: makeEngineCommands() });
		await expect(daemon.client.send('run-flow', null)).rejects.toThrow();
	});

	it('run-flow throws when flowRef is missing', async () => {
		await using daemon = await createTestDaemon({ commands: makeEngineCommands() });
		await expect(daemon.client.send('run-flow', { notFlowRef: 'x' })).rejects.toThrow();
	});

	it('cancel throws when runId is missing', async () => {
		await using daemon = await createTestDaemon({ commands: makeEngineCommands() });
		await expect(daemon.client.send('cancel', { notRunId: 'x' })).rejects.toThrow();
	});
});
