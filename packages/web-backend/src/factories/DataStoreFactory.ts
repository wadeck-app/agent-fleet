...
export class DataStoreFactory {
	...
	getIngredientsService(): IngredientsService {
		if (!this.ingredientsService) {
			...
			const baseRepo = new BaseRepository<Ingredient>('ingredients', this.storage);
	...
	getBooksService(): BooksService {
		if (!this.booksService) {
			// Create BaseRepository
			const baseRepo = new BaseRepository<Book>('books', this.storage);
	...
	async seedData(): Promise<void> {
		if (this.storage instanceof InMemoryStorage) {
			// Seed ingredients
			await this.storage.seed<Ingredient>('ingredients', [
				{
					id: '1',
					name: 'Apple',
					calories: 95,
			...
			// Seed books
			await this.storage.seed<Book>('books', [
				{
					id: '1',
			...
			//console.log('[FACTORY] Seeded initial data');
		}
	}
}
