	...
	private render(): void {
		...
		if (this.logs.length === 0) {
			...
			for (const log of this.logs) {
				...
				this.screenBuffer.put({ x: 3, y: logY, attr: { color: 'gray', dim: true } }, truncate(log, width - 6));
				logY++;
			}
		}
		...
		this.screenBuffer.put({ x: 3, y: currentY, attr: { color: 'cyan' } }, `┤ Tasks (${this.tasks.length}) ├`);
		...
		if (this.tasks.length === 0) {
			...
			const tasksByStatus = this.tasks.reduce(
				(acc, task) => {
					if (!acc[task.status]) {
						acc[task.status] = [];
					}
			...
			for (const [status, statusTasks] of Object.entries(tasksByStatus)) {
				...
				this.screenBuffer.put(
					{ x: 3, y: taskY, attr: { color, bold: true } },
				...
				for (const task of statusTasks as Task[]) {
					...
					if (task.flowId && taskY < currentY + contentHeight - 1) {
						...
						this.screenBuffer.put(
							{ x: 3, y: taskY, attr: { color: 'gray', dim: true } },
		...
		this.screenBuffer.put(
			{ x: leftWidth + 4, y: currentY, attr: { color: 'cyan' } },
		...
		if (this.workers.length === 0) {
			this.screenBuffer.put(
				{ x: leftWidth + 4, y: workerY, attr: { color: 'gray', dim: true } },
			...
			for (const worker of this.workers) {
				...
				this.screenBuffer.put(
					{ x: leftWidth + 4, y: workerY, attr: { color: 'green', bold: true } },
					`Worker ${workerIdShort}`
				);
				this.screenBuffer.put(
					{ x: leftWidth + 4 + 15, y: workerY, attr: { color: 'gray', dim: true } },
					`(${worker.type})`
				);
				workerY++;
				...
				if (currentTask) {
					...
					this.screenBuffer.put({ x: leftWidth + 6, y: workerY, attr: { color: 'yellow' } }, `▶ ${taskDesc}`);
					workerY++;
					this.screenBuffer.put(
						{ x: leftWidth + 8, y: workerY, attr: { color: 'gray', dim: true } },
						`[${currentTask.status}]`
					);
				} else {
					this.screenBuffer.put({ x: leftWidth + 6, y: workerY, attr: { color: 'gray', dim: true } }, 'Idle');
				}
				workerY++;
		...
		// ═══ Footer ═══
		this.screenBuffer.put({ x: 2, y: currentY, attr: { color: 'gray', dim: true } }, 'Press Q to quit');
