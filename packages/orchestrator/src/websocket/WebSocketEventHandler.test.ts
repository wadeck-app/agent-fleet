...
describe('WebSocketEventHandler', () => {
  ...
  describe('TASK_FAILED Message Handling', () => {
    ...
    it('should log error', () => {
      ...
      expect(Logger.error).toHaveBeenCalledWith(
        expect.stringContaining('failed task task-1')
      );
    });
  });
  ...
  describe('Flow Step Messages', () => {
    ...
    it('should handle FLOW_STEP_STARTED message', () => {
      const stepStartedMessage: FlowStepStartedMessage = createMessage(
        MessageType.FLOW_STEP_STARTED,
        {
          workerId: 'worker-1',
          taskId: 'task-1',
          stepId: 'step-1',
    ...
    it('should handle FLOW_STEP_STARTED without step name', () => {
      ...
      expect(mockTaskManager.addComment).toHaveBeenCalledWith(
        'task-1',
        'system',
        'Flow step started: step-1'
      );
    });

    it('should handle FLOW_STEP_COMPLETED message', () => {
      const stepCompletedMessage: FlowStepCompletedMessage = createMessage(
        MessageType.FLOW_STEP_COMPLETED,
        {
          workerId: 'worker-1',
          taskId: 'task-1',
          stepId: 'step-1',
    ...
    it('should handle FLOW_STEP_COMPLETED without outputs', () => {
      ...
      expect(mockTaskManager.addComment).toHaveBeenCalledWith(
        'task-1',
        'system',
        'Flow step completed: step-1'
      );
    });

    it('should handle FLOW_STEP_FAILED message', () => {
      const stepFailedMessage: FlowStepFailedMessage = createMessage(
        MessageType.FLOW_STEP_FAILED,
        {
          workerId: 'worker-1',
          taskId: 'task-1',
          stepId: 'step-1',
  ...
  describe('Workspace Messages', () => {
    ...
    it('should handle WORKSPACE_ALLOCATED message', () => {
      const allocatedMessage: WorkspaceAllocatedMessage = createMessage(
        MessageType.WORKSPACE_ALLOCATED,
        {
          workerId: 'worker-1',
          taskId: 'task-1',
          workspaceId: 'ws-1',
    ...
    it('should handle WORKSPACE_RELEASED message', () => {
      const releasedMessage: WorkspaceReleasedMessage = createMessage(
        MessageType.WORKSPACE_RELEASED,
        {
          workerId: 'worker-1',
          taskId: 'task-1',
          workspaceId: 'ws-1',
