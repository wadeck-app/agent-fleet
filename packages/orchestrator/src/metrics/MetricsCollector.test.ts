...
import { StateManager, StateEvent } from 'shared-common/StateManager.js';
import { Task, TaskStatus, WorkerType, WorkerInfo } from 'shared-common/types.js';
...
describe('MetricsCollector', () => {
  ...
  describe('metrics accuracy', () => {
    it('should count APPROVED tasks as completed', () => {
      vi.mocked(mockTaskManager.getAllTasks).mockReturnValue([
        { ...mockTasks[0], status: TaskStatus.APPROVED }
      ]);
    ...
    it('should count BLOCKED tasks as failed', () => {
      vi.mocked(mockTaskManager.getAllTasks).mockReturnValue([
        { ...mockTasks[0], status: TaskStatus.BLOCKED }
      ]);
