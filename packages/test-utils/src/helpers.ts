 ...
 */
export async function captureConsoleOutput<T>(fn: () => T | Promise<T>): Promise<{
	result: T;
	stdout: string[];
	stderr: string[];
}> {
 ...
 */
export async function assertThrowsAsync(fn: () => Promise<any>, messagePattern?: string | RegExp): Promise<Error> {
	try {
		await fn();
 ...
 */
export async function withTimeout<T>(fn: () => Promise<T>, timeoutMs: number, message?: string): Promise<T> {
	return Promise.race([
		fn(),
