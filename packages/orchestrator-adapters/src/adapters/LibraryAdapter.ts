 ...
 * Using 'any' since orchestrator is not a direct dependency
 */
...
export class LibraryOrchestratorAdapter implements OrchestratorClient {
	...
	constructor(orchestrator: Orchestrator) {
		if (!orchestrator) {
			throw new Error('LibraryOrchestratorAdapter requires an Orchestrator instance');
		}
	...
	async getStats(): Promise<OrchestratorStats> {
		const taskManager = this.orchestrator.getTaskManager();
		const wsServer = this.orchestrator.getWsServer();
		const startTime = this.orchestrator.startTime || new Date();
	...
	on<T extends O2BEventType>(event: T, handler: (data: O2BEventData<T>) => void): void {
		const taskManager = this.orchestrator.getTaskManager();
		const stateManager = taskManager.stateManager;
