...
export class SplitView {
	static render(screenBuffer: any, state: UIState, termWidth: number, termHeight: number): void {
		...
		screenBuffer.put(
			{ x: 3, y: leftY++, attr: { color: 'cyan', bold: true } },
		...
		for (const log of displayLogs) {
			...
			screenBuffer.put(
				{ x: leftWidth + 4, y: rightY, attr: { color: 'gray' } },
				formatTime(log.timestamp).substring(0, 8)
			);
			screenBuffer.put({ x: leftWidth + 13, y: rightY }, emoji);
			screenBuffer.put(
				{ x: leftWidth + 13 + emoji.length, y: rightY, attr: { color } },
		...
		screenBuffer.put(
			{ x: 10 + retryCount.toString().length, y: currentY + 1, attr: { color: connected ? 'green' : 'red' } },
