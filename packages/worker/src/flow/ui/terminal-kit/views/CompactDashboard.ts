...
import { formatDuration, formatTime, getStepStatusIcon, getLogEmoji, getLogLevelColor, progressBar, formatPercentage, truncate } from '../../shared/formatters.js';
...
export class CompactDashboard {
  static render(screenBuffer: any, state: UIState, termWidth: number, termHeight: number): void {
    ...
    screenBuffer.put({ x: termWidth - 20, y: currentY + 1, attr: { color: 'cyan' } }, formatDuration(elapsedSeconds));
    screenBuffer.put({ x: termWidth - 10, y: currentY + 1, attr: { color: retryCount > 0 ? 'yellow' : 'white' } }, `↻ ${retryCount}`);
    ...
    screenBuffer.put({ x: termWidth - 30, y: currentY + 2, attr: { color: 'gray' } }, `(${currentStepIndex + 1}/${totalSteps})`);
    screenBuffer.put({ x: termWidth - 24, y: currentY + 2, attr: { color: 'cyan' } }, progressText);
    screenBuffer.put({ x: termWidth - 6, y: currentY + 2, attr: { color: 'cyan' } }, formatPercentage(currentStepIndex + 1, totalSteps));
    ...
    if (displayLogs.length === 0) {
      screenBuffer.put({ x: Math.floor(termWidth / 2) - 10, y: logY + 3, attr: { color: 'gray' } }, 'No logs yet...');
    } else {
      for (const log of displayLogs) {
        ...
        screenBuffer.put({ x: 13 + emoji.length, y: logY, attr: { color } }, log.message.substring(0, termWidth - 20));
        logY++;
      }
    }
    ...
    const workspace = workspaceDir ? (workspaceDir.length > 20 ? '...' + workspaceDir.slice(-20) : workspaceDir) : 'No workspace';
    screenBuffer.put({ x: 3, y: currentY + 1, attr: { dimColor: true } }, workspace);
    screenBuffer.put({ x: termWidth - 40, y: currentY + 1, attr: { color: connected ? 'green' : 'red' } }, connected ? 'Connected' : 'Disconnected');
