...
export function drawDoubleBox(screenBuffer: any, x: number, y: number, width: number, height: number, color: string): void {
...
export function drawSingleBox(screenBuffer: any, x: number, y: number, width: number, height: number, color: string): void {
  screenBuffer.put({ x, y, attr: { color } }, '┌' + '─'.repeat(width - 2) + '┐');
  for (let i = 1; i < height - 1; i++) {
    screenBuffer.put({ x, y: y + i, attr: { color } }, '│');
    screenBuffer.put({ x: x + width - 1, y: y + i, attr: { color } }, '│');
