...
import { formatDuration, formatTime, formatStepDuration, getLogEmoji, getLogLevelColor, truncate } from '../../shared/formatters.js';
...
export class TimelineView {
  static render(screenBuffer: any, state: UIState, termWidth: number, termHeight: number): void {
    ...
    screenBuffer.put({ x: 3, y: currentY + 1 },
    ...
    if (taskId && steps.length > 0) {
      ...
      screenBuffer.put({ x: Math.floor(termWidth / 2) - 10, y: currentY + 5, attr: { color: 'gray' } }, 'No active flow');
    }
    ...
    if (displayLogs.length === 0) {
      screenBuffer.put({ x: Math.floor(termWidth / 2) - 10, y: logY + 2, attr: { color: 'gray' } }, 'No output yet...');
    } else {
      for (const log of displayLogs) {
        ...
        screenBuffer.put({ x: 3 + emoji.length, y: logY, attr: { color } }, log.message.substring(0, termWidth - 10));
        logY++;
      }
    }
    ...
    screenBuffer.put({ x: 9, y: currentY }, `${steps.filter(s => s.status === 'completed').length}/${steps.length} steps`);
    ...
    screenBuffer.put({ x: termWidth - 20, y: currentY, attr: { color: connected ? 'green' : 'red' } }, `│ Connected ${connected ? '✓' : '✗'}`);
