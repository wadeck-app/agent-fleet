// task-updater-entry.ts -- background auto-update entry point
// This module is bundled separately as task-updater.cjs.
// It must NOT import any task runtime modules
...
const PKG_NAME = process.env['UPDATER_PKG_NAME'] ?? '@wadeck-app/task-cli';
...
export async function main(): Promise<void> {
	const cliName = PKG_NAME.replace(/^@[^/]+\//, '').replace(/-cli$/, ''); // 'task'
	...
	try {
		...
		try {
			currentVersion = __TASK_CLI_VERSION__;
		} catch {
			return;
		}
		...
		try {
			const bundleFile = `task.cjs`; // self-check step
...
const isEntryPoint =
	...
	(process.argv[1].endsWith('task-updater-entry.js') ||
		process.argv[1].endsWith('task-updater-entry.ts') ||
		process.argv[1].endsWith('task-updater.cjs'));
