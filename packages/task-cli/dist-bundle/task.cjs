...
var TaskConfigLoader = class _TaskConfigLoader {
  ...
  static isInitialized(projectDir) {
    ...
    return fs3.existsSync(path3.join(dir, ".task"));
  }
  static load(options) {
...
var TaskStore = class {
  ...
  create(title) {
    ...
    fs4.writeFileSync(this.taskFilePath(id), JSON.stringify(record, null, 2), "utf8");
  ...
  ensureDirectory() {
    fs4.mkdirSync(this.tasksDir, { recursive: true });
  }
  taskFilePath(id) {
    return path4.join(this.tasksDir, `${id}.json`);
  }
  indexFilePath() {
    return path4.join(this.tasksDir, "index.json");
  }
  readIndex() {
    ...
    try {
      return JSON.parse(fs4.readFileSync(indexPath, "utf8"));
    } catch (err) {
  ...
  writeIndex(index) {
    this.ensureDirectory();
    fs4.writeFileSync(this.indexFilePath(), JSON.stringify(index, null, 2), "utf8");
  }
  addToIndex(summary) {
    const index = this.readIndex();
...
function readChannelFromConfig() {
  try {
    const configFile = path5.join(ConfigDir.get("task"), "config.yml");
...
function getCurrentTaskVersion() {
  try {
    return "2026.08.28-150256-361-9e45a3f4";
  } catch {
...
`);
        ...
        process.stdout.write(
          `[FAIL] ${result.name}${result.error !== void 0 ? ` -- ${result.error}` : ""}
`
        );
      }
    }
...
async function runTaskCliUpdate(opts) {
  if (opts.log) {
    const logFile = path5.join(ConfigDir.get("task"), "update-log.txt");
...
`);
  ...
  (0, import_node_child_process3.execFileSync)("npm", ["install", "-g", `${PKG_NAME}@${previousVersion}`], { stdio: "inherit" });
...
function readBaseVersion() {
  ...
  let hash = "DEV";
  try {
    count = (0, import_node_child_process4.execSync)("git rev-list --count HEAD", { encoding: "utf8" }).trim();
  } catch {
  }
  try {
    hash = (0, import_node_child_process4.execSync)("git rev-parse --short=8 HEAD", { encoding: "utf8" }).trim();
  } catch {
  }
