...
export class LongPollingTransportClient implements ITransportClient {
	...
	private sendSubscriptionMessage(
		...
		fetch(`${this.config.baseUrl}/long-polling/subscription`, {
			method: 'POST',
			headers: {
			...
			.then(response => {
				if (!response.ok) {
					console.error(`[LongPolling] Failed to ${action} events:`, events);
				}
			})
			.catch(error => {
				console.error(`[LongPolling] Error sending subscription:`, error);
			});
	}
	...
	private async performPoll(): Promise<void> {
		...
		try {
			...
			const response = await fetch(`${this.config.baseUrl}/long-polling/events`, {
				method: 'GET',
				credentials: 'include', // Send cookies for authentication
