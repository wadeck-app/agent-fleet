 ...
 *
 * Example Flow:
 * 1. Data2 fetches page 1 -> [items 1-10]
 *    useDataAccumulator -> [items 1-10] (first fetch)
 *
 * 2. Data2 fetches page 2 -> [items 11-20]
 *    useDataAccumulator -> [items 1-20] (accumulated)
 *
 * 3. User changes sort/search -> [items 1-10] (smaller = reset)
 *    useDataAccumulator -> [items 1-10] (reset detected)
 */
...
export function useDataAccumulator<T>(
	...
	useEffect(() => {
		...
		else if (dataState.data !== prevDataRef.current && dataState.data.length > 0) {
			setAccumulated(prev => {
