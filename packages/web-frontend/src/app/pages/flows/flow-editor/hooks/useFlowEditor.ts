...
export function useFlowEditor(flowId: string | undefined) {
	...
	useEffect(() => {
		...
		for (const targetNode of nodesWithPorts) {
			const inputPorts =
				'inputPorts' in targetNode.data && Array.isArray(targetNode.data.inputPorts)
			...
			for (const inputPort of inputPorts) {
				...
				const outputPorts =
					'outputPorts' in sourceNode.data && Array.isArray(sourceNode.data.outputPorts)
	...
	const updateNodeData = useCallback(
		(nodeId: string, updates: Partial<FlowStep> | ConstantNodeData) => {
			setNodes(nds =>
				nds.map(node => {
					...
					// Handle regular step nodes (ensure updates is Partial<FlowStep>)
					const baseStep = 'step' in node.data && node.data.step ? node.data.step : ({} as FlowStep);
					const updatedStep = {
						...baseStep,
						...updates,
					} as FlowStep;
					...
					let updatedPorts = {
						inputPorts: 'inputPorts' in node.data ? node.data.inputPorts : undefined,
						outputPorts: 'outputPorts' in node.data ? node.data.outputPorts : undefined,
					};
