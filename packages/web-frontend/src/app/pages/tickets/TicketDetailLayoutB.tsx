...
interface TicketDetailLayoutBProps {
	ticket: Ticket;
	ticketId: string;
...
/**
 * Layout B (GitHub) - Full-width header + two-column body (main + sidebar)
 */
export function TicketDetailLayoutB({ ticket, ticketId, onUpdate, onRefresh }: TicketDetailLayoutBProps) {
					...
					<div className={dirtyFields.labels ? 'rounded-md ring-2 ring-primary/30 p-2' : ''}>
						...
						<div className="mt-2 flex flex-wrap gap-1">
							{localLabels.map(label => (
								<Badge key={label} variant="outline" className="cursor-pointer text-xs">
									{label}
									<Button
										type="button"
										variant="ghost"
							...
							{localLabels.length === 0 && <p className="text-xs text-muted-foreground">None yet</p>}
						</div>
					</div>
