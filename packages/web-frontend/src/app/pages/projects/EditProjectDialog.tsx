...
const defaultFormData: EditProjectFormData = {
	name: '',
	description: '',
	icon: 'FolderKanban',
	iconColor: '#6366F1',
...
export function EditProjectDialog({ project, open, onOpenChange, onSuccess }: EditProjectDialogProps) {
	...
	const formState = useFormState<EditProjectFormData>({
		...
		onSubmit: async data => {
			...
			try {
				await projectsApi.updateProject(project.id, updateProjectData);
				...
				if (errorMessage.includes('version') || errorMessage.includes('optimistic')) {
					showToast('Project was modified by another user. Please refresh and try again.', 'error');
				} else {
