    ...
    if (root === 'inputs') {
      ...
      if (parts.length < 2) {
        throw new TemplateRenderError(
          'inputs requires a variable name: inputs.varName',
          expression,
          root
        );
      }
      ...
      if (parts.length < 4 || parts[2] !== 'outputs') {
        throw new TemplateRenderError(
          'steps requires format: steps.stepId.outputs.varName',
          expression,
          root
        );
      }
      ...
      if (!stepOutputs) {
        throw new TemplateRenderError(
          `Step '${stepId}' not found or has no outputs`,
          expression,
          stepId
        );
      }
      ...
      if (parts.length < 2) {
        throw new TemplateRenderError(
          'task requires a property: task.priority',
          expression,
          root
        );
      }
  ...
  private resolveNested(
    ...
    for (const key of path) {
      if (current === null || current === undefined) {
        throw new TemplateRenderError(
          `Cannot access '${key}' on null/undefined`,
          fullExpression,
          key
        );
      }
      ...
      if (typeof current !== 'object') {
        throw new TemplateRenderError(
          `Cannot access '${key}' on non-object value`,
          fullExpression,
          key
        );
      }
