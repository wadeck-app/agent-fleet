...
import type { FlowStep, StepTrace, DAG } from '../types.js';
...
export class LoopHandler {
  ...
  public handleLoop(
    ...
    console.log(
      `   📝 Invalidated ${invalidatedSteps.length} step(s): ${invalidatedSteps.join(', ')}`
    );
    if (skippedSteps.length > 0) {
      console.log(
        `   ⏭️  Skipped ${skippedSteps.length} step(s) (skipOnLoop=true): ${skippedSteps.join(', ')}`
      );
    }
  ...
  public handleResetOnSuccess(
    ...
    for (const step of allSteps) {
      if (
        step.onFailure?.goto === completedStepId &&
        step.onFailure?.resetOnSuccess === true
      ) {
