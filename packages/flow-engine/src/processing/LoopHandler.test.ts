...
import type { FlowStep, StepTrace, DAG, DAGNode } from '../types.js';
...
describe('LoopHandler', () => {
    ...
    describe('No onFailure.goto configured', () => {
      it('should return shouldLoop=false when step has no onFailure config', () => {
        ...
        expect(result.reason).toBe(
          'No onFailure.goto configured for this step'
        );
  ...
  describe('handleLoop()', () => {
    ...
    describe('Valid target step', () => {
      ...
      it('should return success=true when target exists in DAG', () => {
        ...
        const result = handler.handleLoop(
          mockStep,
          'step0',
          mockDAG,
          completed,
    ...
    describe('Console logging', () => {
      ...
      it('should log loop information with iteration count', () => {
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith(
          expect.stringContaining("Step 'step1' failed")
        );
        expect(consoleLogSpy).toHaveBeenCalledWith(
          expect.stringContaining("returning to 'step0'")
        );
        expect(consoleLogSpy).toHaveBeenCalledWith(
      ...
      it('should log invalidated steps', () => {
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith(
          expect.stringContaining('📝 Invalidated 2 step(s)')
        );
        expect(consoleLogSpy).toHaveBeenCalledWith(
          expect.stringContaining('step0, step2')
        );
      });
      ...
      it('should log skipped steps when skipOnLoop=true', () => {
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith(
          expect.stringContaining('⏭️  Skipped 1 step(s)')
        );
        expect(consoleLogSpy).toHaveBeenCalledWith(
          expect.stringContaining('skipOnLoop=true')
        );
        expect(consoleLogSpy).toHaveBeenCalledWith(
      ...
      it('should not log skipped steps when none are skipped', () => {
        ...
        const skipLogCall = consoleLogSpy.mock.calls.find((call: any[]) =>
          call[0].includes('⏭️')
        );
      ...
      it('should use default maxIterations in log when not specified', () => {
        ...
        expect(consoleLogSpy).toHaveBeenCalledWith(
          expect.stringContaining('iteration 1/3')
        );
      });
    });
  });
  ...
  describe('handleResetOnSuccess()', () => {
    ...
    it('should log reset information when resetting', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('🔄 Reset iteration counter')
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining("'step1'")
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('(was 5)')
      );
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('resetOnSuccess')
      );
    });
