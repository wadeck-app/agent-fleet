...
describe('ScriptExecutor', () => {
	...
	describe('execute', () => {
		...
		it('should stream without stepId prefix', async () => {
			...
			expect(consoleSpy).toHaveBeenCalledWith(expect.stringMatching(/^\s+\[\d{2}:\d{2}:\d{2}\.\d{3}\] output$/));
		});
	...
	describe('ScriptExecutionError', () => {
		it('should create error with all properties', () => {
			const error = new ScriptExecutionError('Test error', 1, 'output', 'error output');
