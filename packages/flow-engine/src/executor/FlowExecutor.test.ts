...
describe('FlowExecutor', () => {
	...
	describe('FlowExecutionError', () => {
		it('should create error with flow ID only', () => {
			...
			expect(error.message).toBe("Flow execution error in 'my-flow': Something went wrong");
		...
		it('should create error with flow ID and step ID', () => {
			const error = new FlowExecutionError('Step failed', 'my-flow', 'step-1');

			expect(error.message).toBe("Flow execution error in 'my-flow' at step 'step-1': Step failed");
			expect(error.name).toBe('FlowExecutionError');
			expect(error.flowId).toBe('my-flow');
		...
		it('should format message correctly with special characters', () => {
			const error = new FlowExecutionError('Failed: "quotes" and \'apostrophes\'', 'my-flow', 'step-1');
		...
		it('should handle special flow and step IDs', () => {
			const error = new FlowExecutionError('Test', 'flow-with-dashes', 'step_with_underscores');
