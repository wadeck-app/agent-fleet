...
| Package                 | Role                                                                                                                                                                                                                                           |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `packages/orchestrator` | Long-running coordinator. Owns task queue, worker pool, WebSocket server for workers (port ), REST API (port ), intervention routing, and event bridge to web-backend. Can run as a standalone process or embedded inside web-backend. |
...
| `packages/shared-orch-worker`      | Contract layer between orchestrator and worker processes. Typed message envelopes (`OWMessage`, `WOMessage`), task domain types (`TaskStatus`, `Task`), orchestrator event types, `StateManager`.                                                                         |
