...
### Prompt visibility in OS process list (OpenCode only)

**Reason:** OpenCode receives the prompt as a positional CLI argument (`opencode run [message]`), making it visible in `ps aux` / Task Manager to any process running as the same user. Claude Code receives the prompt via stdin and is not affected.
**Risk accepted:** Inherent to the OpenCode CLI design. Mitigation: avoid putting secrets directly in flow prompts; use MCP tools or env vars for sensitive values.

### Windows shell:true fallback when only .cmd shim is available

**Reason:** `opencode.exe` is preferred and resolved first via `where.exe opencode.exe` (shell:false, T-05 compliant). The shell:true fallback only activates when no `.exe` is found and only the `.cmd` shim is present. In that edge case, cmd.exe metacharacter injection is a residual risk.
**Risk accepted:** The .exe path resolves correctly when opencode is installed via Chocolatey or npm global. The .cmd-only scenario (manual install without .exe) is rare and documented.
