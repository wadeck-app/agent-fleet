...
| T-04 | Spoofing | A fake *pool* registers and is asked to provision workers | **Mitigated (Phase 4b).** Source registration carries its own credential: `WorkerSourceRegistry` issues a second, independent secret at declaration and stores only its hash (`sourceTokenHash`). `verifySourceToken` accepts it and refuses a worker token; `verifyToken` refuses the source token. An entry predating the split has no source hash and therefore cannot register as a source at all -- absent is a refusal, never a match | Mitigated | 17 |
...
| T-08 | Elevation of Privilege | Inbound worker runs with the launching user's shell environment, granting a step broader privileges than a daemon-forked worker | **Accepted and documented, not contained.** A forked worker gets an allow-listed environment (`ForkWorkerSource.buildEnv`); `flow worker` inherits the terminal's, which is the point -- it exists so a human's own tools, credentials and TTY are available to a step (D#32, D#48). Stated in `WorkerCommand.ts` so the person launching it knows a step runs with their shell's reach | Accepted (Phase 2a) | D#48 |
...
| ID | Threat category | Threat description | Mitigation | Status | Decision # |
|---|---|---|---|---|---|
| T-11 | Spoofing / Elevation of Privilege | A fake host provider registers and manufactures capacity wholesale, absorbing steps across projects. Far more damaging than a single fake worker | **Mitigated (Phase 4b)** by the separate credential above, plus two structural limits: a host is tracked in `HostRegistry`, never in `WorkerRegistry`, so it is never dispatched to; and it cannot exceed the capacity it declared, which the daemon records and never raises. A refused registration is disconnected, not merely ignored | Mitigated | 17, 19 |
...
| T-13 | Elevation of Privilege | A compromised daemon instructs every connected host to execute code, owning the whole fleet | **Largely eliminated by Decision #19**, and now enforced by the wire format: `DaemonToSource` has exactly one message, `provide_worker`, carrying a request id, projects and labels -- no command, script or path, asserted by test. The host acts on recipes it holds locally. Residual: a compromised daemon can still inflate demand to make hosts spawn workers, bounded by each host's declared capacity | Mitigated by design | 19 |
