...
**Décision :** C-D (renommage kebab → PascalCase) : la règle cible les fichiers de *classes exportées* — `engine-client.ts` et `engine-daemon.ts` n'exportent que des fonctions. Skip justifié.
