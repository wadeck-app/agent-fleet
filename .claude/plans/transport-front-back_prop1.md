    ...
    const poll = async () => {
      try {
        ...
        const response = await fetch(
          `${this.baseUrl}${this.eventConfig.pollingEndpoint}`,
          {
...
export class TransportFactory {
    ...
    switch (config.type) {
      ...
      case 'websocket':
        return new WebSocketTransportClient(
          ...
          { enabled: true, maxRetries: 5, backoff: [1000, 2000, 5000] }
        );
  ...
  static createWithFallback(config: TransportConfig): ITransportClient {
    ...
    if (config.fallback === 'http' && config.type === 'websocket') {
      return new FallbackTransportClient(
        primary,
        new HttpTransportClient(config.http!.baseUrl)
      );
    }
...
export const getTransportConfig = (): TransportConfig => {
  ...
  const type = override || debugMode || (
    process.env.NODE_ENV === 'test' ? 'mock' :
    process.env.NODE_ENV === 'production' ? 'websocket' : 'websocket'
  );
...
export type EventType =
  ...
  | 'worker_connected' | 'worker_disconnected' | 'worker_state_changed'
  | 'system_alert' | 'system_error' | 'system_warning'
  | 'metrics_updated' | 'throughput_changed' | 'latency_changed';
