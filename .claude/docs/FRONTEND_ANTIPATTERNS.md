...
<button className={`btn ${isActive ? 'bg-blue-500' : 'bg-gray-500'} ${isLarge ? 'text-lg' : 'text-sm'}`}>Click</button>
```
...
```tsx
// ✅ Using cn()
<button className={cn('btn', isActive ? 'bg-primary' : 'bg-secondary', isLarge && 'text-lg')}>Click</button>
```
...
function TaskCard({ task }: { task: TaskViewModel }) {
	return (
		<div>
			...
			<Badge className={task.status === 'done' ? 'bg-green-500' : 'bg-red-500'}>{task.status}</Badge>
		</div>
	);
}
```
...
<div className="w-[1200px] lg:w-[800px] md:w-[600px] sm:w-[400px]">
	<div className="grid grid-cols-4 lg:grid-cols-3 md:grid-cols-2 sm:grid-cols-1">{items}</div>
</div>
```
...
<div className="w-full max-w-screen-xl mx-auto px-4">
	<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">{items}</div>
</div>
```
