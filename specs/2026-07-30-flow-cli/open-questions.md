...
- The model *chooses* to inject or not — not forced
...
- Injection scope: steps are injected as sub-tasks *within* the generating step — the step is not "done" until its injected sub-steps complete. Step B (depends on A) waits for A AND all of A's injected sub-steps. This enables feedback loops (e.g. "I touched JS, inject run-tests if not already present").
