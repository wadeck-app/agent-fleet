...
export const rule: Rule<Config> = {
    ...
    tags: 'ts',
    defaultScope: [
        'packages/flow-cli/src/cli/**/*.ts',
        'packages/flow-cli/src/utils/loadYaml.ts',
    ],
    ...
    async check(files: string[]): Promise<Violation[]> {
        ...
        const OUTPUT_PATTERNS = [
            /console\.(error|warn|log)\s*\(/,
            /process\.(stderr|stdout)\.write\s*\(/,
        ];
        ...
        for (const file of files) {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                if (!hasRawErr) continue;

                violations.push({
                    file,
                    line: i + 1,
                    message:
                        `Raw error details in user-facing output: replace with a human-friendly message. ` +
