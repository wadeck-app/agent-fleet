 ...
 */
function findClosingBrace(source, openPos) {
    let depth = 0;
 ...
 */
function extractNamedBlocks(block) {
    const result = [];
...
async function loadPluginManifest(pluginDir) {
    ...
    try {
        ...
        if (implHeaderMatch) {
            ...
            if (implClosePos !== -1) {
                ...
                for (const { name: epName, body: epBody } of epBlocks) {
                    if (epName === 'version' || epName === 'provider' || epName === 'sensitiveFields')
                        continue;
        ...
        return {
            ...
            manifest: {
                ...
                manifestVersion: manifestVersionMatch?.[1],
...
function extractPluginId(dirName) {
...
export const rule = {
    ...
    async check(files) {
        ...
        for (const pluginDir of pluginDirs) {
            ...
            if (!registry || !manifest.implementations)
                continue;
            for (const [extPoint, impls] of Object.entries(manifest.implementations)) {
                ...
                for (const [implName, impl] of Object.entries(impls)) {
                    ...
                    if (impl.version !== undefined) {
                        ...
                        if (!supportedVersions.includes(impl.version)) {
                            violations.push({
                                file: path.join(pluginDir, 'plugin.config.ts'),
                                line: 1,
            ...
            if (manifest.implementations?.['workspace']) {
                ...
                for (const srcFile of srcFiles) {
                    ...
                    // Skip exempt files
                    if (srcContent.includes('@plugin-009-exempt'))
                        continue;
