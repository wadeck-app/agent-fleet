...
export class TaskService {
  ...
  async getActiveTasks(): Promise<Task[]> {
    ...
    return tasks.filter(task =>
      [TaskStatus.IN_PROGRESS, TaskStatus.TESTING, TaskStatus.REVIEWING, TaskStatus.REVIEW].includes(task.status)
    );
  }
  ...
  async getPendingTasks(): Promise<Task[]> {
    const tasks = await this.getAllTasks();
    return tasks.filter(task =>
      [TaskStatus.BACKLOG, TaskStatus.TODO, TaskStatus.REFINING].includes(task.status)
    );
  }
