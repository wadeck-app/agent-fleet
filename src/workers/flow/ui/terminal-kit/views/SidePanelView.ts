...
import { formatDuration, formatTime, formatStepDuration, getStepStatusIcon, progressBar, formatPercentage, truncate, getLogEmoji, getLogLevelColor } from '../../shared/formatters.js';
...
export class SidePanelView {
  static render(screenBuffer: any, state: UIState, termWidth: number, termHeight: number): void {
    ...
    // ETA calculation
    const avgStepTime = steps.filter(s => s.durationMs).reduce((sum, s) => sum + (s.durationMs || 0), 0) / Math.max(1, completedSteps);
    ...
    screenBuffer.put({ x: 3, y: leftY++, attr: { color: connected ? 'green' : 'red' } }, connected ? 'Connected ●' : 'Disconnected ○');
    leftY++;
    ...
    screenBuffer.put({ x: 13, y: leftY - 1, attr: { color: retryCount > 0 ? 'yellow' : 'white' } }, retryCount.toString());
    leftY++;
    ...
    if (steps.length === 0) {
      ...
      for (const step of steps) {
        ...
        screenBuffer.put({ x: 5, y: leftY, attr: { color: step.status === 'running' ? 'yellow' : 'white' } }, truncate(step.name, 15));
        screenBuffer.put({ x: 22, y: leftY, attr: { color: 'cyan', dimColor: true } },
    ...
    screenBuffer.put({ x: 13, y: leftY - 1, attr: { color: errorCount > 0 ? 'red' : 'white' } }, errorCount.toString());
    ...
    if (displayLogs.length === 0) {
      screenBuffer.put({ x: leftWidth + Math.floor(rightWidth / 2) - 10, y: logY + 5, attr: { color: 'gray' } }, 'No logs yet...');
    } else {
      for (const log of displayLogs) {
        ...
        screenBuffer.put({ x: leftWidth + 3, y: logY, attr: { color: 'gray' } }, formatTime(log.timestamp).substring(0, 8));
        screenBuffer.put({ x: leftWidth + 12, y: logY }, emoji);
        screenBuffer.put({ x: leftWidth + 12 + emoji.length, y: logY, attr: { color } },
    ...
    if (paused) {
      screenBuffer.put({ x: leftWidth + 2 + hint.length, y: hintY, attr: { color: 'yellow', bold: true } }, ' [PAUSED]');
    }
  }
}
