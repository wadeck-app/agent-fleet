...
export class WorkspaceManager {
  ...
  private async validateManualWorkspace(workspacePath: string): Promise<void> {
    ...
    if (!fs.existsSync(workspacePath)) {
      throw new WorkspaceAllocationError(
        `Manual workspace path does not exist: ${workspacePath}`
      );
    }
  ...
  public async allocate(options: WorkspaceAllocationOptions): Promise<Workspace> {
    ...
    if (config.mode === 'manual') {
      if (!existingPath) {
        throw new WorkspaceAllocationError(
          'Manual mode requires existingPath to be provided'
        );
      }
    ...
    if (config.mode === 'shared') {
      ...
      if (config.reusePolicy === 'always' || config.reusePolicy === 'if-available') {
        const reusableWorkspace = await this.findReusableWorkspace(
          taskId,
          config,
          gitBranch
        );
    ...
    throw new WorkspaceAllocationError(
      `Unsupported workspace mode: ${config.mode}`
    );
  }
  ...
  private async setupGitWorktree(
    ...
    try {
      ...
      throw new WorkspaceAllocationError(
        `Failed to create git worktree: ${error}`
      );
    }
  }
