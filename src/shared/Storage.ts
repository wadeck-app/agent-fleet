...
export class Storage {
  ...
  static async saveTask(task: Task): Promise<void> {
    try {
      ...
      throw new Error(`Failed to save task ${task.id}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async deleteTask(taskId: string): Promise<void> {
    try {
      ...
      throw new Error(`Failed to delete task ${taskId}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async addKnowledge(category: string, entry: Omit<KnowledgeEntry, 'timestamp' | 'category'>): Promise<void> {
    try {
      ...
      throw new Error(`Failed to add knowledge to ${category}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async readKnowledge(category: string): Promise<KnowledgeEntry[]> {
    try {
      ...
      throw new Error(`Failed to read knowledge from ${category}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async getTaskContextDir(taskId: string): Promise<string> {
    try {
      ...
      throw new Error(`Failed to get context directory for task ${taskId}: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  static async clearAllTasks(): Promise<number> {
    try {
      ...
      await Promise.all(
        jsonFiles.map(file =>
          fs.promises.unlink(path.join(TASKS_DIR, file))
        )
      );
