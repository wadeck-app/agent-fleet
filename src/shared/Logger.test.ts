...
import { Logger, LogLevel, StructuredLogEntry } from './Logger.js';
...
describe('Logger', () => {
  ...
  describe('logStructured', () => {
    ...
    it('should include workerId from context', () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringContaining('worker:worker-456')
      );
    });
  ...
  describe('integration scenarios', () => {
    it('should support logging from different components', () => {
      ...
      const messages = vi.mocked(mockStateManager.emitLogMessage).mock.calls.map(call =>
        JSON.parse(call[0]) as StructuredLogEntry
      );
